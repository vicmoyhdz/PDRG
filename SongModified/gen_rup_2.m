...
for iter=1:rup.num
    ...
    % rup.slip2.dist{iter} = interp2(X1,Z1,rup.slip2.dist{iter},X,Z,'spline');
    % rup.Vr2.dist{iter} = interp2(X1,Z1,rup.Vr2.dist{iter},X,Z,'spline');
    % rup.psv2.dist{iter} = interp2(X1,Z1,rup.psv2.dist{iter},X,Z,'spline');
...
for iter=1:rup.num
  ...
  rup.p1.sig1(iter,3) = std(rup.psv.dist{iter}(:));
   ...
   %%% Peak Time - Computed but not used for exponential STF
...
for iter=1:rup.num
  ...
  for k=1:rup.nz
    for i=1:rup.nx
      ...
      % mrf(ind,i1:i2) = Mo(k,i)*rup.svf.svf{iter}{k}{i};
    end
  end
