...
rup.mech='ds'; %dip-slip %ss for strike-slip
rup.rake_cov=0; %to add a slight variation to rake, std in degrees (0 no variation)
rup.ref = [530016.7664	4505390.882	-2000]; %upper right corner 
...
wavelength_cut=500;
%The file consists of a matrix with slip values in meters
Deterministic=load('C:\Users\vmh5\OneDrive - Menntaský\Irpinia\Irpinia_1faglia\New_Ameri.srcmod');
...
% 60% of the slip is located in the upper 80% and central 70% of the fault area. 
% Can be changed inside Create Slip

% [slip_nontaper, slip,slipp] = CreateSlip(WL, rup.target_Mw, rup.mu_vec, rup.sampling, cor_len, rup.acf_type, taper_width, taper_function,seed);
...
% 
[slip_nontaper, slip,slipp,radial_spectra,Deterministic_resampled] = CreateSlipModified(Deterministic,wavelength_cut,max_slip,WL, rup.target_Mw, rup.mu_vec, rup.sampling, cor_len, rup.acf_type, taper_width, taper_function,seed);
rup.slip_deterministic=slip;
rup.slip_nontaper=slip_nontaper;
 %----------------------------------------------------------------
...
rup.shyp = 5000;      % along strike location (from top center) of hypocenter
rup.dhyp = 6928;        % along dip location (from top edge) of hypocenter
%-------------------------------------------------------------
 %Hypo coordinates
